...
<div class="row vertical-gap sm-gap justify-content-center">
    <div class="col-sm-6">
        <div class="card">
            <div class="card-body mnt-6 mnb-6">
                <div class="mb-3" :class="{ 'has-error': errors.name }">
                    <label for="name">Name</label>
                    <input class="form-control" id="name" required type="text" placeholder="Enter name" v-model="team.name" />
                    <p class="text-danger" v-if="errors.name">
                        {{ errors.name[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.address }">
                    <label for="address">Alamat Team</label>
                    <input class="form-control" id="address" required type="text" placeholder="Enter Address" v-model="team.address" />
                    <p class="text-danger" v-if="errors.address">
                        {{ errors.address[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.phone }">
                    <label for="phone">Phone Team</label>
                    <input class="form-control" id="phone" required type="tel" placeholder="Enter phone" v-model="team.phone" />
                    <p class="text-danger" v-if="errors.phone">
                        {{ errors.phone[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.tempat_lahir }">
                    <label for="tempat_lahir">Tempat Lahir Team</label>
                    <input class="form-control" id="tempat_lahir" required type="text" placeholder="Enter Tempat Lahir" v-model="team.tempat_lahir" />
                    <p class="text-danger" v-if="errors.tempat_lahir">
                        {{ errors.tempat_lahir[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.tanggal_lahir }">
                    <label for="tanggal_lahir">Tanggal Lahir Team</label>
                    <input class="form-control" id="tanggal_lahir" required type="date" placeholder="Enter Tanggal Lahir" v-model="team.tanggal_lahir" />
                    <p class="text-danger" v-if="errors.tanggal_lahir">
                        {{ errors.tanggal_lahir[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.gender }">
                    <label for="gender">Gender</label>
                    <select id="gender" class="form-control" required v-model="team.gender">
    ...
    <div class="col-sm-6">
        <div class="card">
            <div class="card-body mnt-6 mnb-6">
                <div class="mb-3" :class="{ 'has-error': errors.email }">
                    <label for="email">Email Team</label>
                    <input class="form-control" id="email" required type="email" placeholder="Enter email" v-model="team.email" />
                    <p class="text-danger" v-if="errors.email">
                        {{ errors.email[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.username }">
                    <label for="username">Username</label>
                    <input class="form-control" id="username" type="text" required placeholder="Enter username" v-model="team.username" />
                    <p class="text-danger" v-if="errors.username">
                        {{ errors.username[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.password }">
                    <label for="password">Password</label>
                    <input class="form-control" id="password" type="password" placeholder="Enter password" v-model="team.password" />
                    <p v-if="this.$route.name == `teams.edit`">*Kosongkan jika tidak ingin mengganti</p>
                    <p class="text-danger" v-if="errors.password">
                        {{ errors.password[0] }}
                    </p>
                </div>
                <div class="mb-3" :class="{ 'has-error': errors.position_id }">
                    <label for="position_id">Position</label>
                    <select id="position_id" class="form-control" required v-model="team.position_id">
                        <option selected disabled value="">Select Position</option>
                        <option v-for="row in positions.data" :key="row.name" :value="row.id">
                            {{ row.name }}
                        </option>
                    </select>
                ...
                <div class="mb-3" v-if="team.position_id == 12 || team.position_id == 14 || team.position_id == 15" :class="{ 'has-error': errors.parent_id }">
                    <label for="parent_id">Nama ADV</label>
                    <select id="parent_id" class="form-control" v-model="team.parent_id">
